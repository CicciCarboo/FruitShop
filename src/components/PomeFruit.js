...
const fetchPomefruit = async () => {
        ...
        const pomefruitData = await response.json();
        setPomefruit(pomefruitData);
    } /* After been initialized, this function must be invoked within useEffect to actually do something */
    
useEffect(() =>{
    
    if(!pomefruit.length){ /*this if-statement makes sure that this function is not invoked over and over again*/
        fetchPomefruit();
    }
})
    ...
    return(
        <>
            <h4 className="fruit-category">Pome fruit</h4>        
            <ul className="fruits">{
                pomefruit.map( fruit => (
